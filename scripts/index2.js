...
function setInLocalStorage(nameVal, gender) {

   // saving to local storage in form (key=name, value=gender)
   localStorage.setItem(nameVal, gender);  
   console.log("name=" + nameVal + " gender= " + gender + " stored in local storage." );
  }
 
  /*
  deleteFromLocalStorage():
  this function deletes record using name of user from local storage.
  ***NOTE that some logs are printed on the inspect-elements in browser
  you can see this logs for step by step oprations.
  */
 
function deleteFromLocalStorage(nameVal) {

    // removing name of user from local storage
   var storedGender = getFromLocalStorage(nameVal)
   localStorage.removeItem(nameVal);
   console.log("name=" + nameVal + " gender= " + storedGender + " removed from storage." );
  }

  /*
    getFromLocalStorage():
    this function passes the key (name of user) and retrives the 
    he/she's gender.
  */
 
function getFromLocalStorage(nameVal) {  // search in local storage by name.
   var storedGender = localStorage.getItem(nameVal);
   return storedGender;
  }
      ...
      */
         ...
         */
         ...
         if(validInput && genderValue !== "")           // if has valid input and gender value store it.
            setInLocalStorage(nameValue, genderValue); // saving true values from user. pair(name, gender)
   
         // search for cookie name and if it was cookie for that name, show in TextBox.
         
         storageValue = getFromLocalStorage(nameValue);

         if(storageValue !== null){   // if user had saved information.
            let savedMessageTextBox = document.getElementById("savedAnswerTextBox");
            savedMessageTextBox.value = storageValue;
         }
         
      })
         .catch(err =>{
            createErrorMsg("Error! Network problem.");
            validInput = false});   // error handeling. (network problem!!)
      }
...
clearBtn.addEventListener("click", ()=>{
   ...
   resetSavedMsgTextBox();
   deleteFromLocalStorage(nameValue);
});
